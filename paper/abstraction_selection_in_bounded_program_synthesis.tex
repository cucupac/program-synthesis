...
\summary{Reusable abstractions can shorten programs. Some do not improve
bounded search. We compare two selection procedures on a synthetic grid benchmark.
...
\begin{abstract}
Reusable abstractions can make program synthesis more efficient by replacing
several operations with one library item. However, each item also expands the
solver's search space. With a fixed budget, this added search can prevent the
solver from reaching a shorter solution. We ask when abstraction libraries
improve bounded synthesis and what determines their value. The answer matters
because future savings must justify selection and added search costs.
