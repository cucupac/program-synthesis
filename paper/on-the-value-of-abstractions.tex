...
\]
where each $P_{i,\mathrm{prim}}$ solves its problem $T_i$. Both selection procedures choose from the same set of 50 abstraction
candidates, $A_{\mathrm{candidates}}$, found through primitive-only search.
Section~\ref{sec:abstraction-candidates} describes how we construct this set,
and Figure~\ref{fig:solver-experience} illustrates the process.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[conceptfig]
    ...
    \node[procnode,text width=3.2cm] (filter) at (5.7,-1.9)
      {Count targets reachable\\in one hop;\\require at least two};
    ...
    \draw[->] (grids) -- (filter);
    \draw[->] (filter) -- node[midway,above,note] {keep the top 50 by count} (acand);
  \end{tikzpicture}
  \caption{Construction of the candidate set shared by both selection
  procedures. Section~\ref{sec:abstraction-candidates} describes the
  one-hop test and ranking procedure.}
...
For each subprogram output state, we ask if there are any target grids
that can be reached with just one additional operation (i.e., ``one hop'').
If at least two target grids can be reached in one hop, we record the
subprogram as a potential abstraction candidate. We rank the candidates
by the number of target grids that can be reached in one hop and keep
the top 50.

The target grids come from $T_{\mathrm{prim}}$. The additional operation
can invert or reflect the candidate's output grid, or use add or subtract
to combine it with another reached grid. Ties are broken by the attempt
at which the primitive-only solver first reached the candidate's grid,
then by program string. Construction uses no problems from
