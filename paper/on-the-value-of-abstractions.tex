...
\]
where each $P_{i,\mathrm{prim}}$ solves its problem $T_i$. To construct
our abstraction candidate set, we first collect the distinct grids reached
during primitive-only search and the programs that produce them. We then
look for grids that show potential for reuse across the problems in
$T_{\mathrm{prim}}$. In other words, we ask whether a grid could help
construct the target for more than one problem. After filtering and ranking
these programs, we keep 50 as our \emph{abstraction candidates}, which form
$A_{\mathrm{candidates}}$. Both selection procedures choose from this same
set. Figure~\ref{fig:solver-experience} illustrates this process, and
Section~\ref{sec:abstraction-candidates} describes the filtering and ranking
procedure.

\begin{figure}[H]
  \centering
  \begin{tikzpicture}[conceptfig]
    \node[procnode,text width=2.3cm] (search) at (0,0)
      {Primitive-only\\search};
    \node[setnode,text width=3.2cm] (grids) at (5.7,0)
      {Distinct reached grids\\and their programs};
    \node[procnode,text width=3.2cm] (filter) at (5.7,-1.9)
      {Filter for reuse across\\$T_{\mathrm{prim}}$ and rank};
    \node[setnode,text width=2.3cm] (acand) at (0,-1.9)
      {50 candidates\\$A_{\mathrm{candidates}}$};
    \draw[->] (search) -- node[midway,above,note] {collect} (grids);
    \draw[->] (grids) -- (filter);
    \draw[->] (filter) -- node[midway,above,note] {keep the top 50} (acand);
  \end{tikzpicture}
  \caption{From primitive-only search to abstraction candidates. We first
  collect the distinct grids reached during search and the programs that
  produce them. We then check whether each grid could help construct targets
  across the primitive-only problems, and use this potential for reuse to
  filter and rank the programs. The top 50 form the candidate set
  $A_{\mathrm{candidates}}$ shared by both selection procedures.}
...
\label{sec:abstraction-candidates}

To construct $A_{\mathrm{candidates}}$, we first collect grids reached
during primitive-only search, then filter the programs that produce them
based on their potential for reuse. The procedure below describes these two
stages and how we rank the remaining programs to choose 50 candidates.
Each candidate is a subprogram. If selected, the solver stores the
