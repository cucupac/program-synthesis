...
item also expands a solver's search space. With a fixed budget, this added
...
size-ordered search. Across 30 random problem worlds, both procedures choose
...
counts as one attempt, including a duplicate that the solver later discards.
...
grids through the full budget. We keep nonblank, nonprimitive grids whose first
program has one to four operations. A grid supports a starter target when one
invert or reflection produces that target or when one add or subtract with
another reached grid produces it. We keep grids that
...
abstractions are later selected. Abstraction candidate discovery does not use
