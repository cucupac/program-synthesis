...
\]
where each $P_{i,\mathrm{prim}}$ solves its problem $T_i$. We collect
abstraction candidates during primitive-only search, then filter and rank
them for potential reuse across $T_{\mathrm{prim}}$. The top 50 form
$A_{\mathrm{candidates}}$, as described in
Section~\ref{sec:abstraction-candidates}.
Figure~\ref{fig:solver-experience} illustrates this process.
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{From primitive-only search to abstraction candidates. Programs
  producing distinct reached grids are filtered and ranked for potential
  reuse across $T_{\mathrm{prim}}$. The top 50 form
...
The experiment constructs $A_{\mathrm{candidates}}$ from grids reached
during primitive-only search, filtering and ranking their programs to keep
50 candidates in two stages.
